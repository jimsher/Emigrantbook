...
function loadMessages(targetUid) {
    ...
    // 1. ეს ხაზი ხმოვანის გაგზავნისთვისაა - აღდგენილია!
    window.currentChatId = targetUid;
    currentChatId = targetUid;

    // 2. ეს ბლოკი წაშლილი მესიჯების დასამალადაა - აღდგენილია!
    db.ref(`users/${myUid}/deleted_messages/${chatId}`).on('value', deletedSnap => {
        ...
        db.ref(`messages/${chatId}`).on('value', snap => {
            ...
            snap.forEach(child => {
                ...
                // ფილტრი: თუ წაშლილი გაქვს, არ დახატოს
                ...
                // ხმოვანის კონტენტი - შენი ორიგინალი სტილით
...
// წაშლის ფუნქციაც აგერაა, რომ არ გაქრეს
