        ...
        db.ref(`users/${myUid}/deleted_messages/${chatId}`).on('value', deletedSnap => {
            ...
            db.ref(`messages/${chatId}`).on('value', snap => {
                ...
                messagesArray.forEach((item, index) => {
                    ...
                    let content = "";
                    if (isImg) {
                        content = `<img src="${msg.image}" style="max-width:220px; height:auto; border-radius:12px; cursor:pointer;" onclick="window.open('${msg.image}', '_blank')">`;
                    } else if (msg.audio) {
                    ...
                    box.innerHTML += `
                        <div style="display: flex; flex-direction: column; margin-bottom: 8px; width: 100%; align-items: ${isMine ? 'flex-end' : 'flex-start'};" 
                            ...
                            ${isMine && msg.seen && index === messagesArray.length - 1 ? 
                                `<img src="${tPhoto}" style="width:14px; height:14px; border-radius:50%; margin-top:4px; margin-right:2px; border:1px solid var(--gold, #d4af37); flex-shrink:0;">` : ''}
                        </div>`;
                });
