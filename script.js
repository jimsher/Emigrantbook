...
async function sendPushNotification(targetUid, title, message, type = "video_call") {
    // 1. ვიღებთ ადრესატის Token-ს ბაზიდან
    const snap = await db.ref(`users/${targetUid}/fcmToken`).once('value');
    const token = snap.val();

    if (!token) {
        console.log("ადრესატს არ აქვს ჩართული შეტყობინებები (Token missing)");
        return;
    }

    // 2. ვამზადებთ შეტყობინებას
    const payload = {
        to: token,
        notification: {
            title: title,
            body: message,
            icon: myPhoto || "/logo.png",
            click_action: window.location.origin // სადაც გადაიყვანს დაჭერისას
        },
        data: {
            type: type, // მესიჯია თუ ვიდეო ზარი
            callerName: myName,
            url: window.location.origin
        },
        priority: "high" // აუცილებელია, რომ ტელეფონმა მაშინვე გაიღვიძოს
    };

    // 3. ვაგზავნით (ამისთვის საჭიროა Firebase-ის სერვერული გასაღები, მაგრამ სატესტოდ ასეც იმუშავებს)
    fetch('https://fcm.googleapis.com/fcm/send', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'Authorization': 'key=შენი_SERVER_KEY_FIREBASE-დან' // ნახე ქვემოთ სად იპოვო
        },
        body: JSON.stringify(payload)
    }).then(res => console.log("Push გაიგზავნა!"))
