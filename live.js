...
async function joinLive(channelName) { // ამოვიღოთ hostUid პარამეტრი, რადგან ბაზიდან წამოვიღებთ
    ...
    let currentHostUid = null; // ჰოსტის ID-ს შესანახად
    db.ref(`lives_active/${channelName}`).once('value', snap => {
        const liveData = snap.val();
        if(liveData) {
            currentHostUid = liveData.hostId || liveData.uid; // ვიმახსოვრებთ ჰოსტის ID-ს
    ...
    try {
        ...
        // --- აი აქ ჩაემატა სტუმრის გასვლის კონტროლი ---
        liveClient.on("user-left", (user) => {
            updateLiveLayout(false); // სტუმარი რომ გავა, ეკრანი ჩამოდის დაბლა
        });

        liveClient.on("user-published", async (user, mediaType) => {
            await liveClient.subscribe(user, mediaType);
            if (mediaType === "video") {
                // თუ ეს ვიდეო არის ჰოსტის (ვის ლაივშიც ვართ)
                if (user.uid == currentHostUid) {
                    user.videoTrack.play("remote-live-video");
                } 
                // თუ სხვა ვინმეა (სტუმარი)
                else {
                    window.currentGuest = user; // ვიმახსოვრებთ სტუმარს
                    updateLiveLayout(true); // ეკრანი იწევა მაღლა
                    
                    // ჰოსტის ვიდეოს ხელახლა გაშვება, რომ არ გაშავდეს
