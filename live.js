...
async function joinLive(channelName) {
    ...
    // 1. ჰოსტის UID-ის წამოღება (აუცილებელია, რომ მაყურებელმა გაარჩიოს ვინ არის ჰოსტი)
    ...
    try {
        await liveClient.leave(); 
        await liveClient.setClientRole("audience"); // მაყურებლის როლი
        ...
        liveClient.on("user-published", async (user, mediaType) => {
            ...
            if (mediaType === "video") {
                // თუ შემოსული ვიდეო ჰოსტისაა -> დიდ ეკრანზე
                if (user.uid == currentHostUid) {
                    user.videoTrack.play("remote-live-video");
                } 
                // თუ შემოსული ვიდეო სხვისია (სტუმრის) -> აწიე ეკრანი და ჩასვი პატარაში
                else {
                    updateLiveLayout(true); 
                    user.videoTrack.play("guest-remote-video");
                    
                    // დამატებითი დაზღვევა: ჰოსტის ვიდეოც ხელახლა გაუშვი, რომ არ გაშავდეს
        ...
        // თუ ვინმე გადის, ეკრანი ჩამოდის ყველასთან
