...
function drawProductCard(id, item, grid) {
    ...
    const wishlistBtn = `
        <div onclick="event.stopPropagation(); toggleWishlist('${id}', this.querySelector('i'))" 
             style="position:absolute; top:10px; right:10px; z-index:1001; background:rgba(0,0,0,0.7); width:32px; height:32px; border-radius:50%; display:flex; align-items:center; justify-content:center; border:1px.2); backdrop-filter:blur(5px);">
...
function renderUserOrderHistory() {
    ...
    db.ref('promoCodes').once('value', pSnap => {
        ...
        let vipCardHtml = ""; // აქ შევინახავთ კოდის HTML-ს

        if (allCodes) {
            Object.entries(allCodes).forEach(([code, details]) => {
                // თუ კუპონი აქტიურია და 'forUser' ველში წერია მომხმარებლის სახელი
                if (details.active && details.forUser === userName) {
                    vipCardHtml = `
                        <div class="vip-status-card" style="margin-bottom:20px; background: rgba(212,175,55,0.1); border: 1px solid var(--gold); padding: 15px; border-radius: 15px; width: 100%; box-sizing: border-box;">
                            <div class="vip-badge" style="background:var(--gold); color:black; padding:2px 8px; border-radius:4px; font-size:10px; font-weight:bold; display:inline-block; margin-bottom:10px;">👑 VIP წევრი</div>
                            <div style="color:white; font-size:15px; font-weight:bold;">თქვენ გაქვთ პერსონალური VIP სტატუსი</div>
                            <p style="color:gray; font-size:11px; margin-top:5px;">თქვენი პირადი კუპონი:</p>
                            <div class="vip-promo-box" style="display:flex; justify-content:space-between; align-items:center; background:rgba(0,0,0,0.3); padding:10px; border-radius:10px; margin-top:10px; border:1px dashed var(--gold);">
                                <b style="color:var(--gold); font-size:18px; letter-spacing:1px;">${code}</b>
                                <small style="color:#00ff00; font-weight:bold;">-${details.discount}%</small>
                            </div>
                        </div>
                    `;
                }
        ...
        // 2. ვამყარებთ კავშირს შეკვეთებთან
        db.ref('orders').off(); 
        db.ref('orders').on('value', snap => {
            const data = snap.val();
            
            // საწყისი სათაური + VIP ბარათი (თუ მოიძებნა)
